...
@section('content')
    ...
    <form method="POST" action="{{ route('anamnesis.update', $anamnesis->id) }}" accept-charset="UTF-8"
                    ...
                    <div class="form-group">
                        <ul class="list-unstyled">
                            @foreach ($enfermedades as $value)
                                <label>
                                    <input class="name" {{ in_array($value->id, $role1) ? "checked='checked'" : '' }}
                                        name="enfermedades[]" type="checkbox" value="{{ $value->id }}">
                                    {{ $value->Nombre }}
                                </label>
                            @endforeach
                        </ul>
